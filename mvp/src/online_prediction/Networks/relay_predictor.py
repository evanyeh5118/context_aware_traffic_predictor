...
# Go up 3 levels to get to mvp root
mvp_root = os.path.abspath(os.path.join(current_dir, "../../.."))
# Go up 4 levels to get to project root (where the main 'src' module is)
project_root = os.path.abspath(os.path.join(current_dir, "../../../.."))
# Add project_root FIRST so the correct 'src' module is found (needed for unpickling)
if project_root not in sys.path:
    sys.path.insert(0, project_root)
# Also add mvp src directory for direct imports
...
configPath = os.getenv(
    'CONFIG_PATH', 
    os.path.join(mvp_root, "config", "config.json")
)
modelFolder = os.getenv(
    'MODEL_FOLDER',
    os.path.join(mvp_root, "model")
)
verbose = os.getenv(
    'verbose',
    0
...
class RelayPredictor:
    ...
    def _save_config(self, timestamp):
        ...
        config_path = os.path.join(mvp_root, "data", "capture", config_filename)
        #-----------------
        config = json.load(open(configPath))
    ...
    def _save_results(self, timestamp):
        ...
        output_dir = os.path.join(mvp_root, "data", "capture")
