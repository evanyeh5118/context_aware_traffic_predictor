...
# Path to Python components (default to network directory)
PY_DIR="${SCRIPT_DIR}/../src/network"
PY_DIR_PREDICTOR="${SCRIPT_DIR}/../src/online_prediction/Networks"
...
# Check if required Python files exist in network directory
...
if [ ! -f "$PY_DIR_PREDICTOR/relay_predictor.py" ]; then
    echo "Warning: relay_predictor.py not found in $PY_DIR_PREDICTOR"
    echo "Predictor mode will not be available"
fi
...
if [ "$USE_PREDICTOR" = "1" ]; then
    ...
    python3 "$PY_DIR_PREDICTOR/relay_predictor.py" &
else
