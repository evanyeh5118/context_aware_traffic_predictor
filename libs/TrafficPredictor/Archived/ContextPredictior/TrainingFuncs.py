...
def trainModel(parameters, trainData, testData, modelExt=None, optimizerExt=None, verbose=False):
    model, criterion, optimizer, train_loader, test_loader, device = prepareTraining(
        parameters, trainData, testData, verbose=verbose)
    if modelExt is not None:
        model = modelExt
    if optimizerExt is not None:
        optimizer = optimizerExt
...
def trainModelHelper(parameters, model, criterion, optimizer, device, train_loader, test_loader, verbose=False):
    ...
    return checkpoint, avg_train_loss_history, avg_test_loss_history
...
def createOptimizer(model, params):
